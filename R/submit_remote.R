...
submitRemote <-
  function(credentials, remote.folder, script.file, submission.file, data, quiet =
    ...
    checkScriptFileExists(remote.folder,script.file,username,password,host =
                            host,port = port)

    # Check that submission.file is in remote directory
    checkSubmissionFileExists(
      remote.folder,submission.file,username,password,host = host,port = port
    )
...
### Check that server can be accessed
checkFolderExists <-
  function(directory,username,password,host = "lyra.qut.edu.au",port = 22) {
